...
exports['rpstack-persistence']['rpstack:persistence:registerMigration'](
  'rpstack-economy',
  '001_create_economy_accounts',
  [[
    CREATE TABLE IF NOT EXISTS `rpstack_economy_accounts` (
...
exports['rpstack-persistence']['rpstack:persistence:registerMigration'](
  ...
  [[
    CREATE TABLE IF NOT EXISTS `rpstack_economy_transactions` (
      `id`            INT UNSIGNED   NOT NULL AUTO_INCREMENT,
      `char_id`       INT UNSIGNED   NOT NULL,
...
AddEventHandler('rpstack:persistence:ready', function()
  ...
  -- When a new character is created, initialize their economy account
  AddEventHandler('rpstack:identity:characterCreated', function(char_id)
    RPSTACK_ECONOMY_REPO.findByCharacter(char_id, function(existing)
      if existing then return end -- already has an account
